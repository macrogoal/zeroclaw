...
**已开始（切片）：** 版本化 `SessionRecord`（磁盘 v2，自交互 JSON v1 迁移）、`SessionCompactionMeta`（归档相对路径与摘要摘录）、压缩时将片段写入 `~/.zeroclaw/sessions/archives/*.jsonl`（若存在主目录）。**仍待完成：** 与 gateway/daemon/SQLite 的统一恢复、保留/GC、动态上下文重载的显式测试。
