...
**已交付（切片）：** `zeroclaw mcp serve` — stdio MCP（`2024-11-05`），通过 `[mcp_serve]` 与 `--allow-tool` 做工具白名单；默认仅安全只读工具（`memory_recall`、`file_read`），除非 `relax_tool_policy = true`。不反向暴露外部 MCP 客户端已连接的工具。HTTP 传输延后。
