...
**Đã bắt đầu (lát cắt):** `SessionRecord` có phiên bản (v2 trên đĩa, migrate từ JSON tương tác v1), `SessionCompactionMeta` (đường dẫn tương đối tới archive + đoạn tóm tắt), compaction ghi JSONL vào `~/.zeroclaw/sessions/archives/*.jsonl` khi có home. **Chưa xong:** resume thống nhất gateway/daemon/SQLite, retention/GC, test reload dynamic context rõ ràng.
