...
**Đã giao (lát cắt):** `zeroclaw mcp serve` — MCP stdio (`2024-11-05`), allowlist qua `[mcp_serve]` và `--allow-tool`; mặc định chỉ công cụ đọc an toàn (`memory_recall`, `file_read`) trừ khi `relax_tool_policy = true`. Không proxy lại công cụ từ MCP client bên ngoài. HTTP transport để sau.
